...
class DeferredMedia extends HTMLElement {
  ...
  loadContent(focus = true) {
    ...
    if (!this.getAttribute('loaded')) {
      ...
      if (poster) {
        poster.style.transition = 'opacity 0.2s ease';
      }

      // Add loading spinner to the poster button
      if (posterButton) {
        ...
        `;
        posterButton.appendChild(loadingSpinner);
      }
      ...
      setTimeout(() => {
        ...
        if (posterButton) {
          const spinner = posterButton.querySelector('.loading__spinner');
